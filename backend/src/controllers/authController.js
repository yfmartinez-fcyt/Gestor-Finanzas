...
const generateAccessToken = (user, sessionId) => {
    return jwt.sign(
        { id: user.id, email: user.email, rol: user.rol, sid: sessionId },
...
const register = async (req, res) => {
    try {
        ...
        const { nombre, apellido, email, password, avatar } = validation.data;
        ...
        const result = await pool.query(
            'INSERT INTO usuarios (nombre, apellido, email, password, avatar) VALUES ($1, $2, $3, $4, $5) RETURNING id, nombre, apellido, email, avatar, rol, created_at',
            [nombre, apellido, email, hashedPassword, avatar || null]
        );
...
const refresh = async (req, res) => {
    try {
        ...
        jwt.verify(oldRefreshToken, process.env.JWT_REFRESH_SECRET, async (err, decoded) => {
            ...
            // 3. Buscar usuario
            const result = await pool.query('SELECT id, nombre, apellido, email, avatar, rol FROM usuarios WHERE id = $1', [decoded.id]);
            if (result.rows.length === 0) {
...
const getMe = async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT id, nombre, apellido, email, avatar, rol, created_at FROM usuarios WHERE id = $1', [req.user.id]);
